...
/// <summary>
/// NguonC adapter for phim.nguonc.com JSON API. Vietnamese movie source.
/// </summary>
public class NguonCController : BaseOnlineController
{
    public NguonCController() : base(ModInit.conf) { }
        ...
        bool similar = false)
    {
        ...
        if (string.IsNullOrWhiteSpace(slug))
        {
            ...
            if (!similar)
            {
                NcMovie exact = FindExact(search.Value, title, original_title, year);
                if (exact != null)
    ...
    [HttpGet, Staticache(manually: true)]
    [Route("lite/nguonc/video")]
    [Route("lite/nguonc/video.m3u8")]
    ...
    async Task<CacheResult<List<NcMovie>>> Search(string query)
    {
        string cacheKey = $"nguonc:search:{query}";

        return await InvokeCacheResult<List<NcMovie>>(cacheKey, TimeSpan.FromHours(2), textJson: true, onget: async e =>
        {
            string url = $"{ApiBase}/api/films/search?keyword={HttpUtility.UrlEncode(query)}";
            var root = await httpHydra.Get<NcSearchResponse>(url, safety: true, textJson: true);
    ...
    async Task<CacheResult<NcDetailResponse>> Detail(string slug)
    {
        string cacheKey = $"nguonc:detail:{slug}";

        return await InvokeCacheResult<NcDetailResponse>(cacheKey, TimeSpan.FromHours(2), textJson: true, onget: async e =>
        {
            string url = $"{ApiBase}/api/film/{HttpUtility.UrlEncode(slug)}";
            var root = await httpHydra.Get<NcDetailResponse>(url, safety: true, textJson: true);
    ...
    string ApiBase
        => (init?.apihost ?? init?.host ?? "https://phim.nguonc.com").TrimEnd('/');

    SimilarTpl SearchTemplate(
        IEnumerable<NcMovie> movies,
        string title,
        ...
        bool rjson)
    {
        ...
        foreach (var movie in list)
        {
            ...
            string link = $"{host}/lite/nguonc?slug={HttpUtility.UrlEncode(movie.slug)}" +
    ...
    MovieTpl RenderMovie(NcDetailResponse detail, string title, string originalTitle, string slug)
    {
        ...
        bool rjson)
    {
        ...
        if (season == -1 && seasons.Count > 1)
        {
            ...
            foreach (int item in seasons)
            {
                tpl.Append(
                    $"{item} сезон",
                    $"{host}/lite/nguonc?slug={HttpUtility.UrlEncode(slug)}&title={encodedTitle}" +
        ...
        foreach (var candidate in candidates)
        {
            ...
            vtpl.Append(
                ...
                $"{host}/lite/nguonc?slug={HttpUtility.UrlEncode(slug)}&title={encodedTitle2}" +
    ...
    {
        return accsArgs(
            $"{host}/lite/nguonc/video?uri={EncryptQuery(source)}" +
            $"&title={HttpUtility.UrlEncode(title)}&slug={HttpUtility.UrlEncode(slug)}"
        );
    }

    List<int> GetSeasons(NcDetailResponse detail)
    {
        var seasons = new HashSet<int>();
        foreach (var server in detail?.episodes ?? new List<NcEpisodeServer>())
        {
            foreach (var episode in server?.server_data ?? new List<NcEpisode>())
            {
    ...
    List<NcEpisode> EpisodesForSeason(NcEpisodeServer server, short season)
    {
        var episodes = server?.server_data ?? new List<NcEpisode>();
        if (episodes.Count == 0) return new List<NcEpisode>();
    ...
    int EpisodeSeason(NcEpisode episode)
    {
    ...
    int EpisodeNumber(NcEpisode episode)
    {
    ...
    string StreamUrl(NcEpisode episode)
    {
    ...
    static NcMovie FindExact(IEnumerable<NcMovie> items, string title, string originalTitle, short year)
    {
        ...
        foreach (var item in items ?? Enumerable.Empty<NcMovie>())
        {
    ...
    {
        return accsArgs(
            $"/lite/nguonc?slug={HttpUtility.UrlEncode(slug)}&title={HttpUtility.UrlEncode(title)}" +
