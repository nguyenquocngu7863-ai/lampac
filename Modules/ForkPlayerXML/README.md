...
| **GET** `/fxml/cub` | Выдача обычного CUB-каталога по параметрам **`search`**, **`cat`**, **`sort`**, **`page`** через **`http://tmdb.cub.best/`**. Дополнительные query-фильтры, например **`genre`** и **`without_genres`**, прокидываются в CUB, участвуют в cache key и сохраняются в sort/next-page ссылках. См. **`CubController`**. |
...
- Внешний HTTP **`tmdb.cub.best`** для обычных списков в **`/fxml/cub`**.
