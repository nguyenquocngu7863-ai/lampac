...
(function() {
  ...
  window.__lampacSubtitleAutoOwner = 'stremiosub-mx';
  window.__lampacStremioSubMxLoaded = true;
  ...
  /* ── MX Bridge: mo phim ngoai kem sub ── */
  function mxVideoUrl(state) {
    var d = (state && state.data) || {};
    return d.url || d.link || d.file || d.src || '';
  }

  function mxTitle(state) {
    var m = (state && state.info && state.info.movie) || {};
    return m.title || m.name || m.original_title || 'Lampac';
  }

  function openMxBridge(videoUrl, subUrls, title) {
    // Ban dau: ban sub thang trong URL, khong relay, khong menu.
    var u = 'mxbridge://play?video=' + encodeURIComponent(videoUrl);
    if (subUrls && subUrls.length)
      u += '&sub=' + subUrls.map(function(s) { return encodeURIComponent(s); }).join('|');
    u += '&title=' + encodeURIComponent(title || 'Lampac');
    log('open bridge', u.slice(0, 120));
    window.location.href = u;
  }

  /* ── Sub chi la link trung gian (302 moi ra file .srt), MX khong tu theo.
   * Resolve ra URL cuoi bang fetch (response.url) roi moi mo bridge. */
  function resolveFinalUrl(url, callback) {
    try {
      fetch(url, { method: 'HEAD', redirect: 'follow' }).then(function(r) {
        callback(r.url || url);
      }).catch(function() { callback(url); });
    } catch (e) { callback(url); }
  }

  function openMxBridgeResolved(videoUrl, subUrls, title) {
    if (!subUrls || !subUrls.length) {
      openMxBridge(videoUrl, [], title);
      return;
    }
    var pending = subUrls.length;
    var finals = new Array(subUrls.length);
    subUrls.forEach(function(u, i) {
      resolveFinalUrl(u, function(final) {
        finals[i] = final;
        pending--;
        if (pending === 0) openMxBridge(videoUrl, finals, title);
      });
    });
  }

  /* ── MX Bridge relay: day sub len server ngay khi tim xong.
   * App bridge lay ve khi nhan intent (khong doi menu hien sau MX). */
  function postMxOffer(state) {
    try {
      if (!state || !state.rawSubs || !state.rawSubs.length) return;
      if (!serverOrigin) return;
      var videoUrl = mxVideoUrl(state);
      if (!isHttpUrl(videoUrl)) return;
      var subs = state.rawSubs.filter(function(s) { return s && isHttpUrl(s.url); });
      if (!subs.length) return;
      var payload = {
        video: videoUrl,
        title: mxTitle(state),
        subs: subs.map(function(s) { return s.url; })
      };
      if (typeof $ !== 'undefined' && $.ajax) {
        $.ajax({
          url: serverOrigin + '/mxbridge/offer',
          type: 'POST',
        ...
        fetch(serverOrigin + '/mxbridge/offer', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload)
        }).catch(function() {});
      }
      log('mx offer posted', subs.length, 'subs');
    } catch (error) {
      log('mx offer failed:', error.message || error);
    }
  }

  function offerMxBridge(state) {
    // Chi POST am tham len relay, KHONG location.href (WebView bao
    // ERR_UNKNOWN_URL_SCHEME voi scheme la). Bridge tu poll va mo MX.
    postMxOffer(state);
  }
