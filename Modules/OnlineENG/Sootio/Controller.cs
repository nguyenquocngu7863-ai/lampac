...
public sealed class SootioController : BaseOnlineController<ModuleConf>
{
    public SootioController() : base(ModInit.conf) { }
    ...
    )
    {
        ...
        {
            return OnError(
                "Sootio requires a valid IMDb id. A TMDB id is converted through TMDB external_ids; if no exact IMDb mapping exists, Sootio is not queried.",
                400
            );
        }
        ...
        string type = "movie";
        List<SootioStreamItem> allStreams = await GetStreams(type, addonId);
        if (allStreams.Count == 0)
            return OnError("No direct HTTP streams returned by Sootio", 502);

        List<SootioStreamItem> streams = string.IsNullOrWhiteSpace(stream_source)
            ? allStreams
            : allStreams
        ...
        if (streams.Count == 0)
            return OnError("No streams for the selected Sootio source", 404);
    ...
    [Route("lite/sootio/file.mkv")]
    [Route("lite/sootio/file.m3u8")]
    [Route("lite/sootio/file.mp4")]
    public async Task<ActionResult> Video(string u, string h, bool play = true)
    {
        ...
        if (string.IsNullOrWhiteSpace(u))
            return OnError("Missing Sootio stream URL", 400);
        ...
        if (!IsHttpUrl(streamUrl))
            return OnError("Invalid Sootio stream URL", 400);
        ...
        {
            return OnError("Unable to prepare Sootio stream", 502);
        }
    ...
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(addonId))
            return OnError("Sootio requires a valid IMDb series id", 400);
    ...
    )
    {
        ...
        List<SootioStreamItem> streams = await GetStreams(
            "series",
    ...
    async Task<JObject> GetTmdb(string path)
    {
        ...
        string cacheKey = $"sootio:tmdb:{path}";
    ...
    async Task<JObject> GetCinemeta(string addonId)
    {
        ...
        return await InvokeCache(
            $"sootio:cinemeta:{addonId}",
            TimeSpan.FromHours(6),
            () => Http.Get<JObject>(uri, timeoutSeconds: 8, proxy: proxy)
        );
    }

    async Task<List<SootioStreamItem>> GetStreams(string type, string addonId, short expectedSeason = 0)
    {
        var result = new List<SootioStreamItem>();

        if (!Uri.TryCreate(init.manifest, UriKind.Absolute, out Uri manifestUri) ||
            manifestUri.Scheme is not ("http" or "https"))
        {
            Serilog.Log.Warning("Sootio manifest URL is invalid: {Manifest}", init.manifest);
            return result;
        }
        ...
        string cacheKey = $"sootio:streams:{init.manifest}:{type}:{addonId}";
        ...
        foreach (JToken token in streams)
        {
            ...
            // Sootio can return a stream from a different season even though the
            ...
            SootioStreamItem item = ReadStream(stream);
            if (item != null)
                result.Add(item);
        }
    ...
    static bool MatchesExpectedSeason(JObject stream, short expectedSeason)
    {
        ...
        // Sootio currently emits groups such as `...-phan-3`; other providers
    ...
    static SootioStreamItem ReadStream(JObject stream)
    {
        // Sootio returns a donation/banner entry with only externalUrl. Keep
        // accepting the normal Stremio `url` field, but never turn an
        // externalUrl or magnet into a playable item.
        ...
        string fileName = behaviorHints?.Value<string>("fileName") ??
            behaviorHints?.Value<string>("filename") ??
            string.Empty;
        string fullTitle = stream.Value<string>("fullTitle") ?? string.Empty;
        string resolution = stream.Value<string>("resolution") ??
            stream.Value<string>("quality") ??
            string.Empty;
        string size = stream.Value<string>("size") ?? string.Empty;
        string provider = stream.Value<string>("httpProvider") ??
            stream.Value<string>("provider") ??
            string.Empty;
        string bingeGroup = behaviorHints?.Value<string>("bingeGroup") ?? string.Empty;

        if (string.IsNullOrWhiteSpace(provider) ||
            provider.Equals("httpstreaming", StringComparison.OrdinalIgnoreCase))
        {
            provider = Regex.Replace(bingeGroup, @"-streams?$", string.Empty, RegexOptions.IgnoreCase);
        }

        if (string.IsNullOrWhiteSpace(provider))
        {
            Match providerMatch = Regex.Match(
                $"{rawTitle} {rawDescription}",
                @"\|\s*(?<provider>[A-Za-z0-9][A-Za-z0-9 ._-]*)",
                RegexOptions.IgnoreCase
            );
            provider = providerMatch.Success
                ? providerMatch.Groups["provider"].Value
                : "Sootio";
        }

        provider = Compact(provider) ?? "Sootio";
        string metadata = $"{rawUrl} {rawTitle} {rawDescription} {fileName} {fullTitle} {resolution} {size}";
        string quality = FindQuality(metadata);
        string format = DetectFormat(rawUrl, metadata);

        // Sootio's httpstreaming resolver is intentionally opaque (`/resolve/`)
        // while its file name/size/resolution describe a downloadable video.
        // Route that case as MKV so Lampa does not hand an extensionless
        // resolver URL to its HTML5 element.
        if (format == null &&
            (rawUrl.Contains("/resolve/", StringComparison.OrdinalIgnoreCase) ||
        ...
        return new SootioStreamItem(
            rawUrl,
            provider,
            title,
            quality,
            format,
            ReadSubtitles(stream),
    ...
    )
    {
        ...
        foreach (SootioStreamItem stream in streams)
        {
    ...
    )
    {
        ...
        string name = title ?? original_title ?? "Sootio";
        if (season > 0 && episode > 0)
    ...
    string BuildVideoEndpoint(SootioStreamItem stream, bool selectLink = false)
    {
        ...
        string endpoint = $"{host}/lite/sootio/{route}?u={HttpUtility.UrlEncode(EncryptQuery(stream.Url))}";
    ...
    )
    {
        ...
        return accsArgs($"{host}/lite/sootio/episode?{query}");
    }
    ...
    )
    {
        ...
        return accsArgs($"{host}/lite/sootio?{query}");
    }
    ...
    {
        // Sootio's current manifest accepts `tt...` ids, not the generic
        ...
        {
            Serilog.Log.Warning(
                "Sootio rejected conflicting IMDb ids: {Ids}",
                string.Join(",", imdbIds)
            );
            return null;
        }
        ...
        if (!string.IsNullOrWhiteSpace(imdb))
        {
            ...
            // canonical id understood by Sootio.
            if (inputTmdbId > 0)
            {
                ...
                {
                    Serilog.Log.Warning(
                        "Sootio rejected mismatched title ids: TMDB {TmdbId} maps to {MappedImdb}, request supplied {Imdb}",
                        inputTmdbId,
                        mappedImdb,
                        imdb
                    );
        ...
        {
            Serilog.Log.Warning(
                "Sootio could not resolve TMDB {TmdbId} to an IMDb id; stream lookup skipped",
                inputTmdbId
            );
        }
    ...
    static string SourceGroupName(SootioStreamItem stream)
    {
        // The adapter stores Sootio's httpProvider/bingeGroup in Name so the
        // quality list can be grouped by 4KHDHub, 111477, MkvBase, etc. Keep
        // the generic title parser as a fallback for future add-on shapes.
        ...
        return string.IsNullOrWhiteSpace(name) ? "Sootio" : name;
    }
