(function () {
  ...
  function getSeasonEpisode(playData) {
    ...
    if (seasonOnly) return seasonOnly;
    ...
    // vẫn hữu ích cho phim lẻ (luôn là 0/0, không bị sai như phim bộ).
    return { season: playData.season || 0, episode: playData.episode || 0 };
  }
  ...
  function attachAutoSub(playData) {
    ...
    if (imdbId) {
      ...
      fetchAndResolve(imdbId, se.season, se.episode, playData, function (tracks) {
    ...
    Lampa.TMDB.external_imdb_id({ id: tmdbInfo.id, type: tmdbInfo.type }, function (foundImdbId) {
      ...
      var se2 = getSeasonEpisode(playData);
      fetchAndResolve(foundImdbId, se2.season, se2.episode, playData, function (tracks) {
        if (!tracks.length) { log('khong co sub / khong resolve duoc ban nao'); return; }
        safeSetSubtitles(tracks);
      });
    });
