...
Nhánh đang dùng là `arena/01a06710-lampac`. Không clone `main` — `main` đang chậm hơn. Nhánh dự phòng: `arena/01a06710-lampac`.
...
cd ~/lampac
git status
git fetch origin arena/01a06710-lampac
git pull --ff-only origin arena/01a06710-lampac
```
...
git push origin arena/01a06710-lampac
```
...
```bash
git fetch origin arena/01a06710-lampac
git log --oneline origin/arena/01a06710-lampac -5
```
...
```bash
git fetch origin
git push --force-with-lease origin origin/arena/01a06710-lampac:main
```

`origin/arena/01a06710-lampac` là dữ liệu nguồn, còn `:main` là branch đích. `--force-with-lease` chỉ cho phép cập nhật nếu `main` trên remote vẫn đúng phiên bản đã fetch; nó an toàn hơn `--force`. Sau lệnh này, `main` sẽ nhận commit mới nhất của branch agent, ví dụ `11a24fde`.
...
```bash
cd ~/lampac
curl -fL "https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a06710-lampac/setup-termux.sh" -o "$HOME/setup-termux-latest.sh"
...
1. Đảm bảo commit đã nằm trên nhánh `arena/01a06710-lampac` trên GitHub (không chờ `main`).
...
```bash
curl -fsSL "https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a06710-lampac/setup-termux.sh?cb=$(date +%s)" -o ~/setup-termux-branch.sh \
  && bash ~/setup-termux-branch.sh --sync && lampac stop && lampac start
```

Lệnh trên đã trỏ nhánh đang dùng `arena/01a06710-lampac`. Không thay bằng `main`. Dự phòng: `arena/01a06710-lampac`.
