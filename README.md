...
> Nhánh đang dùng: **`arena/01a05da1-lampac`** — nhánh thử nghiệm đang giữ module `VidCore` và bản sửa `lampac update`.
...
> trỏ nhánh `arena/01a05da1-lampac`. Muốn quay lại nhánh cũ `arena/01a05241-lampac`: chỉ cần `export`, không phải sửa script.

Mở Termux, tải script rồi chạy:

```bash
pkg update -y && pkg install -y git curl
git clone --depth 1 --branch arena/01a05da1-lampac https://github.com/nguyenquocngu7863-ai/lampac.git
cd lampac
...
>
> ```bash
> curl -fsSL "https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac/setup-termux.sh?cb=$(date +%s)" -o setup-termux.sh \
...
> Mặc định hiện tại: nhánh `arena/01a05da1-lampac`. Đừng dùng `main`.
> Dự phòng: `arena/01a04e63-lampac`.

```bash
# Bước 0 — lấy script mới (làm một lần sau mỗi patch)
curl -fsSL https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac/setup-termux.sh -o setup-termux.sh
```
...
```text
https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac
```

Kiểm tra đang trỏ đâu:

```bash
echo "${LAMPAC_CUSTOM_SOURCE_BASE:-https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac}"
```

Dùng **một lần** (fork riêng, hoặc quay về nhánh dự phòng 63):

```bash
LAMPAC_CUSTOM_SOURCE_BASE='https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac' \
...
```bash
echo "export LAMPAC_CUSTOM_SOURCE_BASE=https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac" >> ~/.bashrc
source ~/.bashrc
...
set -e
base=https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac
stamp=$(date +%s)
...
1. `base=https://…/arena/01a05da1-lampac` — URL raw **trong Ubuntu**. Giữ nhánh hiện tại; 63 chỉ dự phòng. `export` Termux vẫn dùng cho `--sync`, không cần cho lệnh này.
...
```bash
curl -fsSL https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac/setup-termux.sh | bash -s -- --sync
lampac stop && lampac start
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac/setup-termux.sh \
  | bash -s -- --update
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac/setup-termux.sh | bash -s -- --sync-all
lampac stop
lampac start
```
...
mkdir -p "$d"
base=https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac/Modules/OnlineENG/VidCore
stamp=$(date +%s)
...
proot-distro login ubuntu -- bash -lc '
  ...
  curl -fsSL https://raw.githubusercontent.com/nguyenquocngu7863-ai/lampac/arena/01a05da1-lampac/config/termux-recovery.init.conf \
